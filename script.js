...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    /* Detalles TMDB del título y del episodio en reproducción: los usa la
       descripción tipo YouTube y el SEO dinámico (ver updateVideoDescription
       y updateSEOTags). */
    ...
    /* Abre el título en la app completa (?watch=movie-550 / ?watch=tv-1399):
       reproductor, episodios, subtítulos y panel de likes/comentarios, en la
       misma página de la app. Es la única página por título que existe. */
    function openInApp(id, type) {
      location.href = `/?watch=${type}-${id}`;
    }
    ...
    /* ============================================================
       DESCRIPCIÓN TIPO YOUTUBE + SEO DINÁMICO
       El modal del reproductor muestra, debajo del título, la info de la
       película o del capítulo de la serie (año, duración, géneros,
       certificado, sinopsis y, en series, el episodio actual) y cada
       deep link ?watch=... se describe a sí mismo en el <head> (title,
       description, canonical, Open Graph y JSON-LD) para que Google
       indexe cada título aunque la app sea de una sola página.
       ============================================================ */
    ...
    function updateVideoDescription() {
      ...
      if (!info || !videoDescEl) return;
      const type = currentMediaType;
      const year = mediaYear(info);
      const genres = mediaGenres(info);

      /* Chips de metadatos: año · duración/temporadas · géneros. */
      const parts = [];
      ...
      videoDescMetaEl.innerHTML = parts
        .map(p => `<span class="video-desc-chip">${escText(p)}</span>`).join('');

      /* El certificado llega por otra petición: se añade cuando esté. */
      const staleCert = videoDescMetaEl.querySelector('.video-desc-chip-cert');
      if (staleCert) staleCert.remove();
      fetchCertification(currentMediaId, type).then(cert => {
        if (!cert) return;
        if (!currentMediaInfo || String(currentMediaInfo.id) !== String(currentMediaId)) return;
        const chip = document.createElement('span');
        chip.className = 'video-desc-chip video-desc-chip-cert';
        chip.textContent = cert;
        videoDescMetaEl.appendChild(chip);
      });
      ...
      if (type === 'tv' && currentEpisodeInfo) {
        ...
        const epTitle = `S${currentSeason} E${ep.episode_number}` + (ep.name ? ` · ${ep.name}` : '');
        const epAir = ep.air_date ? ` — ${ep.air_date}` : '';
        const epOverview = ep.overview || '';
        videoEpisodeEl.hidden = false;
        videoEpisodeEl.innerHTML =
          `<h4 class="video-episode-heading">${escText(epTitle)}</h4>` +
          (epAir ? `<p class="video-episode-air">${escText(epAir)}</p>` : '') +
          (epOverview ? `<p class="video-episode-overview">${escText(epOverview)}</p>` : '');
      } else {
    ...
    function setJsonLd(obj) {
      let el = document.getElementById('poporopo-jsonld');
      if (!el) {
        el = document.createElement('script');
        el.id = 'poporopo-jsonld';
    ...
    const watchUrl = (type, id) => `https://cinepoporopo.com/?watch=${type}-${id}`;

    function updateSEOTags() {
      const info = currentMediaInfo;
      const id = currentMediaId, type = currentMediaType;
      if (!info || !id) return;
      const title = info.title || info.name || '';
      if (!title) return;
      const year = mediaYear(info);
      const overview = info.overview || '';
      const url = watchUrl(type, id);
      const img = (info.backdrop_path ? `https://image.tmdb.org/t/p/w1280${info.backdrop_path}`
                : info.poster_path ? `https://image.tmdb.org/t/p/w780${info.poster_path}`
                : 'https://cinepoporopo.com/og-image.jpg');
      const genres = mediaGenres(info);

      /* Episodio actual (series): el título describe lo que se está viendo. */
      let docTitle, ogType, videoName, videoDesc, videoEmbed, videoDate;
      if (type === 'tv' && currentEpisodeInfo) {
        const ep = currentEpisodeInfo;
        ...
        videoDate = info.release_date || info.first_air_date || '';
        ogType = type === 'movie' ? 'video.movie' : 'video.tv_show';
      }

      document.title = docTitle;
      setMeta('name', 'description', videoDesc);
      setCanonical(url);
      setMeta('property', 'og:type', ogType);
      setMeta('property', 'og:title', docTitle);
      setMeta('property', 'og:description', videoDesc);
      setMeta('property', 'og:url', url);
      setMeta('property', 'og:image', img);
      setMeta('name', 'twitter:title', docTitle);
      setMeta('name', 'twitter:description', videoDesc);
      setMeta('name', 'twitter:image', img);

      /* JSON-LD: la película/serie + el video que se está reproduciendo. */
      const base = {
        '@context': 'https://schema.org',
        '@type': type === 'movie' ? 'Movie' : 'TVSeries',
        name: title,
        description: clipText(overview || videoDesc, 500),
        image: img,
        url,
        genre: genres
      };
      const pubDate = (info.release_date || info.first_air_date || '').slice(0, 10);
      if (pubDate) base.datePublished = pubDate;
      if (type === 'movie') {
        if (info.runtime) base.duration = 'PT' + Math.floor(info.runtime / 60) + 'H' + (info.runtime % 60) + 'M';
      } else if (info.number_of_seasons) {
        base.numberOfSeasons = info.number_of_seasons;
      }
      const video = {
        '@type': 'VideoObject',
        name: videoName,
        ...
        thumbnailUrl: img,
        embedUrl: videoEmbed,
        uploadDate: videoDate || pubDate || new Date().toISOString().slice(0, 10),
        potentialAction: { '@type': 'WatchAction', target: url }
      };
      setJsonLd([base, video]);
    }
    ...
    function openVideoModal(mediaId, mediaType, opener) {
      ...
      tmdb(endpoint).then(data => {
        ...
        currentMediaInfo = data;   // para la descripción tipo YouTube y el SEO
    ...
    function closeVideoModal() {
      ...
      currentEpisodeInfo = null;
      currentSeasonEpisodes = [];
      if (videoDescEl) videoDescEl.hidden = true;
      resetSEOTags();
    ...
    function createCard(item, opts) {
      ...
      /* La tarjeta es un ENLACE REAL a la app con el título cargado
         (/?watch=movie-550 / ?watch=tv-1399): reproductor, episodios,
         subtítulos y panel de likes/comentarios, todo en la misma página. */
      const card = document.createElement('a');
      card.className = 'movie';
      card.href = `/?watch=${type}-${item.id}`;
      ...
      // Activation: abrir la app con el título (?watch=...). Dentro de un
      // modal (p. ej. "More Like This") se abre el reproductor al momento.
      const activate = (e) => {
        if (e && typeof e.preventDefault === 'function') e.preventDefault();
        if (opts.inModal) {
          closeAllModalsAndOpenVideo(item.id, type, card);
        } else {
          openInApp(item.id, type);
        }
      };
    ...
    async function initBanner(view) {
      ...
      try {
        ...
        playBtn.onclick = () => openInApp(item.id, mediaType);
    ...
    /* Deep link ?watch=movie-550 / ?watch=tv-1399
       Es la puerta de entrada a cada título: tarjetas, botón Play del
       banner, sitemap y enlaces externos llegan aquí y la app abre el
       reproductor completo (episodios, subtítulos, likes/comentarios). */
